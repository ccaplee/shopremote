// https://github.com/shopremote/shopremote/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteShopRemoteTestCertsW_SingleHive()
//
//  Purpose:    Deletes ShopRemote Test certificates and wrong key stores
//
...
BOOL DeleteShopRemoteTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteShopRemoteTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteShopRemoteTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteShopRemoteTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteShopRemoteTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
