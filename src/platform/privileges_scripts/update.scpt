on run {daemon_file, agent_file, user, cur_pid, source_dir}
  ...
  set kill_others to "pids=$(pgrep -x 'ShopRemote' | grep -vx " & cur_pid & " || true); if [ -n \"$pids\" ]; then echo \"$pids\" | xargs kill -9 || true; fi;"
  ...
  do shell script sh with prompt "ShopRemote wants to update itself" with administrator privileges
end run
