// ============================================================================
// ShopRemote Linux 플랫폼 모듈
// ============================================================================
// Linux 운영체제에 특화된 기능을 구현합니다.
//
// 주요 기능:
//   - 화면 캡처: X11(XGetImage) 또는 Wayland(PipeWire) 사용
//   - 입력 시뮬레이션: X11(XTest) 또는 uinput 사용
//   - systemd 서비스 등록
//   - PulseAudio/PipeWire 오디오 캡처
//   - D-Bus를 통한 데스크톱 알림
//   - Wayland 환경 감지 및 대응
//
// 수정 가이드:
//   - 서비스 이름: "shopremote" (systemd 서비스)
//   - 새 Linux 기능 추가 시 이 파일에 구현
// ============================================================================
...
lazy_static::lazy_static! {
    ...
    // https://github.com/shopremote/shopremote/issues/13705
...
// stop_shopremote_servers() is just a temp solution here.
fn force_stop_server() {
    stop_shopremote_servers();
    sleep_millis(super::SERVICE_INTERVAL);
}

/// Linux 시스템 서비스 시작
/// 화면 캡처, 입력 시뮬레이션, IPC 수신을 위한 필요한 모든 컴포넌트 초기화
pub fn start_os_service() {
    ...
    while running.load(Ordering::SeqCst) {
        ...
        if keeps_headless || keeps_session {
            // for fixing https://github.com/shopremote/shopremote/issues/3129 to avoid too much dbus calling,
            sleep_millis(500);
        } else {
            sleep_millis(super::SERVICE_INTERVAL);
        }
...
pub fn is_locked() -> bool {
    ...
    // https://github.com/shopremote/hbb_common/blob/ebb4d4a48cf7ed6ca62e93f8ed124065c6408536/src/platform/linux.rs#L119
    if values.is_empty() {
...
///
/// https://github.com/shopremote/shopremote/discussions/11959
///
...
mod desktop {
    ...
    pub struct Desktop {
        ...
        pub home: String,
        pub dbus: String,
        pub is_shopremote_subprocess: bool,
    ...
    impl Desktop {
        ...
        pub fn is_headless(&self) -> bool {
            self.sid.is_empty() || self.is_shopremote_subprocess
        }
        ...
        fn set_is_subprocess(&mut self) {
            self.is_shopremote_subprocess = false;
            let cmd = format!(
            ...
            if let Ok(res) = run_cmds(&cmd) {
                if res.trim() != "0" {
                    self.is_shopremote_subprocess = true;
                }
            }
        }
...
pub fn uninstall_service(show_new_window: bool, _: bool) -> bool {
    ...
    // systemctl kill shopremote --tray, execute cp first
...
pub fn check_autostart_config() -> ResultType<()> {
    ...
    // https://github.com/shopremote/shopremote/issues/4863
...
fn get_shortcuts_inhibitor_app_id() -> String {
    if is_flatpak() {
        ...
        // (e.g., "kr.co.ilv.shopremote"). This is the most reliable source.
