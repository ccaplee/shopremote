...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_shopremote() {
            if s.contains("ShopRemote")
                && !name.starts_with("upgrade_shopremote_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("ShopRemote") {
                    s = s.replace("ShopRemote", &app_name);
                } else {
                    // https://github.com/shopremote/shopremote-server-pro/issues/845
                    // If app_name contains "ShopRemote" (e.g., "ShopRemote-Admin"), we need to avoid
                    // replacing "ShopRemote" within the already-substituted app_name, which would
                    // cause duplication like "ShopRemote-Admin" -> "ShopRemote-Admin-Admin".
                    //
