...
#define SHOPREMOTE_LIB_PATH "libshopremote.so"
typedef bool (*ShopRemoteCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_shopremote_core_main() {
   void* libshopremote = dlopen(SHOPREMOTE_LIB_PATH, RTLD_LAZY);
   if (!libshopremote) {
      fprintf(stderr,"Failed to load \"libshopremote.so\"\n");
      char* error;
   ...
   auto core_main = (ShopRemoteCoreMain) dlsym(libshopremote,"shopremote_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"shopremote_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
...
int main(int argc, char** argv) {
  if (!flutter_shopremote_core_main()) {
      return 0;
  }
