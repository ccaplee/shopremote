...
hbb_name = 'shopremote' + ('.exe' if windows else '')
...
def build_flutter_deb(version, features):
    ...
    system2(
        'cp ../res/shopremote-link.desktop tmpdeb/usr/share/applications/shopremote-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/rustdesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/rustdesk/')
    system2(
        'cp ../res/pam.d/shopremote.debian tmpdeb/etc/pam.d/rustdesk')
    system2(
    ...
    system2('dpkg-deb -b tmpdeb shopremote.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('shopremote.deb', '../shopremote-%s.deb' % version)
    os.chdir("..")
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/libshopremote.dylib target/release/shopremote.dylib")
    os.chdir('flutter')
    system2('flutter build macos --release')
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/ShopRemote.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"ShopRemote Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon ShopRemote.app 200 190 --hide-extension ShopRemote.app shopremote.dmg ./build/macos/Build/Products/Release/ShopRemote.app")
    os.rename("shopremote.dmg", f"../shopremote-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libshopremote.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/shopremote.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/shopremote.exe')
    os.chdir('../..')
    if os.path.exists('./shopremote_portable.exe'):
        os.replace('./target/release/shopremote-portable-packer.exe',
                   './shopremote_portable.exe')
    else:
        os.rename('./target/release/shopremote-portable-packer.exe',
                  './shopremote_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/shopremote_portable.exe')
    os.rename('./shopremote_portable.exe', f'./shopremote-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/shopremote-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/shopremote.exe')
        system2('mv target/release/shopremote.exe target/release/ShopRemote.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\shopremote.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/ShopRemote.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/shopremote-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/shopremote-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/shopremote')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv shopremote-%s-0-x86_64.pkg.tar.zst shopremote-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/shopremote-%s-0.x86_64.rpm ./shopremote-%s-fedora28-centos8.rpm' % (
                version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/shopremote-%s-0.x86_64.rpm ./shopremote-%s-suse.rpm' % (
                version, version))
        # yum localinstall rustdesk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/rustdesk*.deb ./flutter/shopremote.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/ShopRemote.app/Contents/MacOS/rustdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/ShopRemote.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/shopremote-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/ShopRemote.app/Contents/MacOS/rustdesk
    #rcodesign sign --p12-file ~/.p12/shopremote-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/ShopRemote.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/shopremote-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/ShopRemote.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/ShopRemote.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/ShopRemote.app
    '''.format(pa))
                system2(
                    'create-dmg "ShopRemote %s.dmg" "target/release/bundle/osx/ShopRemote.app"' % version)
                os.rename('ShopRemote %s.dmg' %
                          version, 'shopremote-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/shopremote-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./shopremote-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./shopremote-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple shopremote-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/ShopRemote.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/rustdesk*.deb ./shopremote.deb')
                system2('dpkg-deb -R shopremote.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/rustdesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                os.system('cp pam.d/shopremote.debian tmpdeb/etc/pam.d/rustdesk')
                system2('strip tmpdeb/usr/bin/rustdesk')
                system2('mkdir -p tmpdeb/usr/share/rustdesk')
                system2('mv tmpdeb/usr/bin/rustdesk tmpdeb/usr/share/rustdesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/rustdesk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb shopremote.deb; /bin/rm -rf tmpdeb/')
                os.rename('shopremote.deb', 'shopremote-%s.deb' % version)
